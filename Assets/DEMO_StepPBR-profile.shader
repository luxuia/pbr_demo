    ...
    SubShader
    {
        ...
        Pass
        {
			...
			//	return fixed4(1,1,1,1);
				for (iter = 0; iter < _ITER_COUNT; ++iter) {
						...
						float3 F = F0 + (1 - F0) * exp2((-5.55473 * vh - 6.98316) * vh);
							...
							float3 DirectLightResult = diffColor + specColor;
								...
								*/
								...
								accum = accum + float4(DirectLightResult + IndirectResult, 1);
								//continue;
								}

								float4 result = fixed4(1/accum.r,1/accum.g, 1/accum.b, 1);//float4(DirectLightResult + IndirectResult, 1);
				return result;
            }
